...
//   };
// };
export const convertUtilityPropsToSX = (styledSystemProps, _descendants, propsWithUtility) => {
  // console.setStartTimeStamp('convertUtilityPropsToSX');

  const sxPropsConvertedObj = {};
  const ignoredProps = {};
  if (Object.keys(propsWithUtility).length === 0) return {
    sxProps: {},
    mergedProps: {}
  };
  const {
  ...
  Object.keys(componentProps).forEach(prop => {
    if (styledSystemProps[prop]) {
      setObjectKeyValue(sxPropsConvertedObj, ['style', prop], componentProps[prop]);
    } else {
      // if (prop !== 'dataSet') {
